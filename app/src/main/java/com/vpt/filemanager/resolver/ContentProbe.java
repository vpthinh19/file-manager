...
public final class ContentProbe {
    ...
    @Inject
    public ContentProbe(LocalStorageAdapter storage, ArchiveHandler archives) {
    ...
    @NonNull
    public ContentKind detectNonArchive(@NonNull File file) throws FileOperationException {
        ...
        if (mp4(bytes, count)) return ContentKind.VIDEO;
        if (mp3(bytes, count) || wav(bytes, count) || ogg(bytes, count) || flac(bytes, count)) {
            return ContentKind.AUDIO;
        }
        if (text(bytes, count)) return ContentKind.TEXT;
        return ContentKind.EXTERNAL;
    }
