...
public final class DocumentSession {
    ...
    public LoadResult load(boolean allowBinary) throws FileOperationException {
        Item entry = inspectFile();
        if (entry.size() > HARD_LIMIT_BYTES) {
        ...
        boolean truncated = entry.size() > READ_ONLY_THRESHOLD_BYTES;
        ReadResult read = readDecoded(truncated ? READ_ONLY_THRESHOLD_BYTES : Integer.MAX_VALUE,
                !truncated);
        baseline = new Baseline(entry.size(), entry.modifiedAt(), read.fingerprint);
    ...
    private Item inspectFile() throws FileOperationException {
    ...
    private boolean looksBinary() throws FileOperationException {
        try (InputStream input = files.openRead(path)) {
    ...
    private ReadResult readDecoded(int limit, boolean fingerprint) throws FileOperationException {
        ...
        try (InputStream raw = files.openRead(path);
    ...
    private boolean matches(Item current) throws FileOperationException {
        if (current.size() != baseline.size || current.modifiedAt() != baseline.modifiedAt) {
            return false;
        }
